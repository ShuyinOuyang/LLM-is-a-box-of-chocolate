...
def solution_evaluation(solution, test_cases, demo_file, time_limit):
    ...
    for i in range(len(test_cases)):
        try:
            ...
            error_case.append('[timeout]' + str(e))
            continue
        except Exception as e:
            print(e, flush=True)
            case_status.append('exception')
            error_case.append('[exception]' + str(e))
            continue
        if output.returncode != 0:
            case_status.append('execution error: %s' % output.returncode)
            error_case.append(output.stderr)
        else:
    ...
    return passed_case, case_status, error_case
